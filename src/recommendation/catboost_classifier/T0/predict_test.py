...
PREDICTION_OUTPUT = 'src/recommendation/catboost_classifier/T0/predictions/transaction_predictions_grouped_catbased.csv'
PREDICTION_OUTPUT_T1 = 'src/recommendation/catboost_classifier/T1/predictions/transaction_predictions_grouped_catbased.csv'
...
def run_predictions(TEST_DATA_PATH,PREDICTION_OUTPUT):
    ...
    prediction_probas['cust_id'] = df[id_col]
    ...
    for category in categories:
        ...
        if not os.path.exists(model_path):
            ...
            prediction_probas[category] = 0.0
            continue
        ...
        prediction_probas[category] = y_proba
        
        print(f"Processed: {category} (Threshold={threshold:.4f})")

    # Save outputs
    os.makedirs(os.path.dirname(PREDICTION_OUTPUT), exist_ok=True)
    binary_predictions.to_csv(PREDICTION_OUTPUT, index=False)
    
    probas_output = PREDICTION_OUTPUT.replace('.csv', '_probas.csv')
    prediction_probas.to_csv(probas_output, index=False)
    
    print(f"\nBinary predictions saved to: {PREDICTION_OUTPUT}")
    print(f"Prediction probabilities saved to: {probas_output}")

    return binary_predictions, prediction_probas
