...
def evaluate_transaction_predictions(transaction_predictions, ans_key, probabilities_df=None, output_folder="results"):
    ...
    # Ranking metrics initialization
    total_ndcg = 0.0
    total_ndcg_with_probs = 0.0 if probabilities_df is not None else None
    ...
    for _, row in merged.iterrows():
        ...
        # Calculate nDCG (using binary predictions as relevance scores)
        pred_order = np.argsort(-pred_array)
        # print('cust_id', cust_id)
        # print('pred_order', pred_order)
        ideal_order = np.argsort(-true_array)
        # print('ideal_order', ideal_order)
        
        dcg = np.sum(true_array[pred_order] / np.log2(np.arange(2, len(pred_order) + 2)))
        idcg = np.sum(true_array[ideal_order] / np.log2(np.arange(2, len(ideal_order) + 2)))
        ...
        # Calculate nDCG with probabilities if available
        if prob_array is not None:
            prob_order = np.argsort(-prob_array)
            # print('prob_array', prob_array)
            # print('prob_order', prob_order)
            dcg_prob = np.sum(true_array[prob_order] / np.log2(np.arange(2, len(prob_order) + 2)))
            ndcg_prob = dcg_prob / idcg if idcg > 0 else 0.0
            total_ndcg_with_probs += ndcg_prob
        
        # Store results
        results.append({
            'cust_id': cust_id,
            'ndcg': ndcg,
            'ndcg_with_probs': ndcg_prob if prob_array is not None else None,
            'hit_rate': hit_rate,
    ...
    metrics['ranking_metrics'] = {
        # 'average_ndcg': total_ndcg / total_customers,
        'average_ndcg_with_probs': total_ndcg_with_probs / total_customers if probabilities_df is not None else None
    }
...
# binary_predictions = pd.read_csv('src/recommendation/catboost/predictions/transaction_predictions_grouped.csv')
# probability_predictions = pd.read_csv('src/recommendation/catboost/predictions/transaction_predictions_grouped_scores.csv')
# ans_key = pd.read_csv('src/data/cf_demog_summary/user_item_matrix/user_item_matrix_grouped.csv')

binary_predictions = pd.read_csv('src/recommendation/catboost_classifier/T0/predictions/transaction_predictions_grouped_catbased.csv')
probability_predictions = pd.read_csv('src/recommendation/catboost_classifier/T0/predictions/transaction_predictions_grouped_catbased_probas.csv')

# binary_predictions_T1 = pd.read_csv('src/recommendation/catboost_classifier/T1/predictions/transaction_predictions_grouped_catbased.csv')
# probability_predictions_T1 = pd.read_csv('src/recommendation/catboost_classifier/T1/predictions/transaction_predictions_grouped_catbased_probas.csv')
...
detailed_results, metrics = evaluate_transaction_predictions(
    binary_predictions, 
    # binary_predictions_T1, 
    ans_key, 
    probabilities_df=probability_predictions,
    # probabilities_df=probability_predictions_T1,
    output_folder="src/recommendation/catboost_classifier/T0/eval_results_grouped_catbased"
    # output_folder="src/recommendation/catboost_classifier/T1/eval_results_grouped_catbased"
)
