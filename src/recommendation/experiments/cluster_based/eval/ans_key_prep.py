...
def group_and_normalize_categories(df, group_custom=True):
    """
    ...
    Returns:
    - DataFrame with normalized transaction counts per customer
    """
    # Create a copy to avoid modifying the original DataFrame
    df = df.copy()
    ...
    new_grouped_categories = [
        ...
        'transport_travel', 'leisure', 'public_services'
    ]
    ...
    final_columns = core_categories + new_grouped_categories if group_custom else [
        ...
        'personal_care', 'medical', 'home_and_living', 'insurance', 'automotive',
        'restaurant', 'business', 'entertainment', 'bank', 'education', 'pet_care',
        'government', 'travel', 'transportation', 'visit'
    ]
...
user_item_matrix.to_csv('src/data/cf_demog_summary/user_item_matrix/user_item_matrix_grouped_catbased.csv', index=False)
