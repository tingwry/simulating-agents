...
PREDICTION_OUTPUT = 'src/recommendation/multioutputclassifier_logreg/T0/predictions/transaction_predictions_grouped_catbased.csv'
...
def run_predictions():
    ...
    print(f"Number of probability arrays: {len(y_proba)}")
    
    # Fill DataFrames
    for i, category in enumerate(categories):
        binary_predictions[category] = y_pred[:, i]

        proba_array = y_proba[i]
        print(f"Category {category}: proba shape = {proba_array.shape}")
