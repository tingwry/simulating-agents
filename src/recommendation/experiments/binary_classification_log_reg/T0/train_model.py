...
MODEL_DIR = 'src/recommendation/binary_classification_log_reg/T0/models'
METRICS_DIR = 'src/recommendation/binary_classification_log_reg/T0/metrics'
RANDOM_STATE = 42
TEST_SIZE = 0.2
...
def train_and_evaluate_models():
    """Train and evaluate models for each category using Logistic Regression"""
    ...
    for category in categories:
        ...
        # Check if we have both classes in the entire dataset
        unique_classes = y.unique()
        if len(unique_classes) < 2:
            print(f"Warning: Category '{category}' has only one class in entire dataset. Skipping...")
            continue
        ...
        if min_class_count < 2:
            print(f"Warning: Category '{category}' has classes with too few samples (min: {min_class_count}). Attempting random split...")
            X_train, X_test, y_train, y_test = train_test_split(
                X, y, test_size=TEST_SIZE, random_state=RANDOM_STATE)
            
            # Check if we ended up with both classes in training and test
            if len(np.unique(y_train)) < 2 or len(np.unique(y_test)) < 2:
                print(f"Warning: After split, category '{category}' has only one class in training or test set. Skipping...")
                continue
        else:
