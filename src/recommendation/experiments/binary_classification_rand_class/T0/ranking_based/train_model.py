    ...
    for category in categories:
        ...
        try:
            ...
            if y_pred_proba.shape[1] == 2:
                ...
                # Only one class predicted, create array of zeros for positive class
                positive_class_proba = np.zeros(len(y_pred))
        except AttributeError:
            # Model doesn't support predict_proba, use predictions as probabilities
            positive_class_proba = y_pred.astype(float)
        ...
        # Save model with metadata including probability capability
        model_data = {
            'model': clf,
            'category': category,
            'preprocessor': preprocessor,
            'can_predict_proba': hasattr(clf, 'predict_proba')  # Only mark True if predict_proba exists
        }
