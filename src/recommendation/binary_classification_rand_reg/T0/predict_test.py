...
def run_predictions():
    ...
    for category in categories:
        ...
        if is_regression:
            ...
            binary_predictions[category] = (pred_counts >= 0.2).astype(int)
            
            # For scores: use actual predicted counts (will be normalized in evaluation)
            prediction_scores[category] = pred_counts
        else:
            # Classification model
            binary_predictions[category] = model.predict(X_all).astype(int)
            
            # Get probabilities if available
            if hasattr(model, 'predict_proba'):
                proba = model.predict_proba(X_all)
                prediction_scores[category] = proba[:, 1] if proba.shape[1] == 2 else binary_predictions[category].astype(float)
            else:
                prediction_scores[category] = binary_predictions[category].astype(float)
        
        print(f"Processed: {category} ({'Regression' if is_regression else 'Classification'})")
...
#     # Generate predictions
#     pred_df, score_df = run_predictions()
    
#     # Load answer key (using actual transaction amounts)
...
#         pred_df,
#         ans_key,
#         probabilities_df=score_df,  # Using scores (counts/probabilities) for ranking
