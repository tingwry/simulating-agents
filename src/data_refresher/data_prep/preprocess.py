...
def preprocess(raw_data, col_selected, col_map, file_name, result_path):
    df = raw_data.copy()
    cust_ids = df['cust_id']
    ...
    numerical_cols_except_age = [col for col in numerical_cols]

    df[numerical_cols_except_age] = df[numerical_cols_except_age].fillna(0.0)
    ...
    # Save csv
    joined_result_path = os.path.join(result_path, file_name)
...
if __name__ == "__main__":
    mock_cust_df = pd.read_csv('src/data_refresher/data/raw_data/mock_customers.csv')
    cleaned_mock_result_path = 'src/data_refresher/data/preprocessed_data'

    train_result_T0_path = 'src/data_refresher/data/T0/train_df.csv'
    test_result_T0_path = 'src/data_refresher/data/T0/test_df.csv'

    train_result_T1_path = 'src/data_refresher/data/T1'
    test_result_T1_path = 'src/data_refresher/data/T1'


    print('Step 1: Preprocess raw data for T0')
    cleaned_mock = preprocess(mock_cust_df, col_selected_T0, col_map_T0, "cleaned_mock.csv", cleaned_mock_result_path)

    print('Step 2: Train test split')
    train_df, test_df = train_test_split(cleaned_mock, test_size=0.2, random_state=42)
    print("Training set shape:", train_df.shape)
    print("Testing set shape:", test_df.shape)

    train_df.to_csv(train_result_T0_path, index=False)
    test_df.to_csv(test_result_T0_path, index=False)
    print(f"\n✅ Train and Test Results saved")

    print('Step 3: Save Train and Test sets for T1')
    # test T1 actual
    filtered_mock_test = mock_cust_df[mock_cust_df['cust_id'].isin(test_df['CUST_ID'])]
    preprocess(filtered_mock_test, col_selected_T1, col_map_T1, "test_T1_actual.csv", test_result_T1_path)
    # train T1
    filtered_mock_train = mock_cust_df[mock_cust_df['cust_id'].isin(train_df['CUST_ID'])]
    preprocess(filtered_mock_train, col_selected_T1, col_map_T1, "train_T1.csv", train_result_T1_path)
