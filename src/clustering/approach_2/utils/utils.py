...
def clustering(train_df, df_combined, MODEL_DIR, n_clusters):
    ...
    existing_models = [f for f in os.listdir(MODEL_DIR) if f.startswith('model_app2_v') and f.endswith('.pkl')]
    if existing_models:
    ...
    model_filename = f"model_app2_v{next_version}.pkl"
...
# Test
def predict(test_df, df_embedding, model):
    # Preserve customer IDs before any processing
    cust_ids = test_df['CUST_ID'].copy()
    test_df = test_df.drop('CUST_ID', axis=1)

    clusters_predict = model.predict(df_embedding)
    ...
    # Add back customer IDs to the non-outlier results
    df['CUST_ID'] = cust_ids[df.index]  # This preserves the correct IDs
